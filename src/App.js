/* By Linda Lim | Autocomplete Widget | BSD 3-Clause License | https://github.com/CookiesNCream/autocomplete-hashtags-mentions-relations */
...
const text = '#hashtags @mention <>relation I am having a blast! This is so awesome and cool! @linda :D';
...
export default class App extends React.Component { /* The App React component is rendered via index.js */
  ...
  constructor(props) {
    ...
    const relationMentionPlugin = createMentionPlugin({
      entityMutability: 'IMMUTABLE',
      mentionPrefix: "<>",
      mentionTrigger: "<",
    });

    this.plugins = [mentionPlugin, hashtagMentionPlugin, relationMentionPlugin];
  ...
  onSearchChange = ({ value }) => {
    this.setState({
      mentionSuggestions: defaultSuggestionsFilter(value, mentions), /* Triggers a match string */
      hashtagSuggestions: defaultSuggestionsFilter(value, hashtags), /* Triggers a match string */
      relationSuggestions: defaultSuggestionsFilter(value, relations), /* Triggers a match string */
    });
  };
  ...
  render() { /* To render the Editor as a React div component to HTML */
    const [mentionPlugin, hashtagMentionPlugin, relationMentionPlugin] = this.plugins;
    const { MentionSuggestions } = mentionPlugin;
    const { MentionSuggestions: HashtagSuggestions } = hashtagMentionPlugin;
    const { MentionSuggestions: RelationSuggestions } = relationMentionPlugin; 
      ...
      <div className="App">
        ...
        <div className="editor" onClick={this.focus}>
          <Editor
            ...
            plugins={this.plugins} /* Parses plugins to the Editor */
